...
**Why she fits Layer 3:** Guinevere turns **organizational risk appetite** and **control selections** into a **policy document** that conforms to the Layer 3 schema: she defines **scope** (in/out dimensions), **imports** (guidance and control catalogs, with optional constraints and assessment-requirement-modifications), **risks** (mitigated and accepted), and **adherence** (evaluation methods, assessment plans, enforcement methods, non-compliance). Her policies reference the controls that Pang (Layer 2) and others have defined, and they can reference Sachiko’s guidance (Layer 1). The result is the governance rule set that Layer 5 evaluations and Layer 7 audit will assess against.
